...
| `gh automagist restart` | Stop the running daemon and start it again on the current `state.json`, so a file you just added or removed takes effect without a separate `stop` then `monitor`. Runs as a daemon by default; `--foreground` keeps it in the terminal, and `--debounce=<dur>` works as it does on `monitor`. Starts cleanly when no daemon was running. |
