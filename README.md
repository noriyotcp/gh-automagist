...
| `gh automagist monitor` | Start the monitor in the foreground. Use `--daemon` to run it silently in the background, or `--debounce=<dur>` to tune the quiet-window before Gist syncs (see [Configuration](#configuration)). On startup it reconciles every tracked file with the same rules as `push`, so edits made while the daemon was down are caught instead of waiting for the next write. |
