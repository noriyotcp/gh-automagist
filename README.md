...
| `gh automagist restart` | Stop the running daemon and start it again — for picking up a new binary after an upgrade, or changing `--debounce`. Adding and removing files needs no restart: the daemon watches `state.json` and follows it live. Runs as a daemon by default; `--foreground` keeps it in the terminal, and `--debounce=<dur>` works as it does on `monitor`. Starts cleanly when no daemon was running. |
